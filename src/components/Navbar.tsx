...
import { Menu, X, Car, Heart, User, Bell } from 'lucide-react';
          ...
          <div className="hidden md:flex items-center gap-8">
            {links.map((link) => (
              <Link key={link.path} to={link.path} className="relative group">
                ...
                <span className="absolute -bottom-1 left-0 h-0.5 transition-all duration-300 origin-left"
                  style={{ width: location.pathname === link.path ? '100%' : '0%', background: location.pathname === link.path ? 'linear-gradient(90deg, hsl(43 96% 56%) 0%, hsl(38 92% 50%) 100%)' : 'transparent' }} />
                <span className="absolute -bottom-1 left-0 h-0.5 w-0 group-hover:w-full transition-all duration-300 origin-left"
                  style={{ background: 'linear-gradient(90deg, hsl(43 96% 56%) 0%, hsl(38 92% 50%) 100%)' }} />
              </Link>
            ))}
